...
function toggleMessage() {
    ...
    renderText(
        isPrincipalExpanded ? fullText : truncatedText,
        "principal-message"
    );
    document.getElementById("toggle-button").innerText = isPrincipalExpanded
        ? "Show less"
        : "Read more";
}
...
function mngrtoggleMessage() {
    ...
    document.getElementById("mntoggle-button").innerText = isManagerExpanded
        ? "Show less"
        : "Read more";
}
...
document.addEventListener("DOMContentLoaded", function () {
    renderText(
        (principalMessageText || "").slice(0, 150) + "...",
        "principal-message"
    );
    renderText(
        (managerMessageText || "").slice(0, 150) + "...",
        "manager-message"
    );
});
