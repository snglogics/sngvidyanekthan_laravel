    ...
    <div class="mb-4" data-aos="fade-up">
        ...
        <form id="uploadForm" enctype="multipart/form-data">
            <div class="mb-3">
                ...
                <input type="text" name="common_header" class="form-control" placeholder="Enter Common Header" required>
            </div>
            <button type="button" class="btn btn-success mb-3" id="addImageBtn">Add Image</button>
            <div id="imageInputs" class="d-flex flex-column gap-3"></div>
            
            <button type="submit" class="btn btn-primary mt-3" id="uploadButton">
                <span class="spinner-border spinner-border-sm d-none me-2" role="status" aria-hidden="true" id="uploadSpinner"></span>
        ...
        @foreach ($galleryImages as $commonHeader => $images)
            <div class="mb-4 border rounded shadow-sm p-3 bg-light">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <form action="{{ route('event.deleteByHeader') }}" method="POST" onsubmit="return confirm('Delete all images under {{ $commonHeader }}?');">
                        @csrf
                        <input type="hidden" name="common_header" value="{{ $commonHeader }}">
                        <button type="submit" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i> Delete Group</button>
                    </form>
                </div>
                <div class="row g-4">
                    @foreach ($images as $image)
                        <div class="col-md-4 col-sm-6" data-aos="fade-up">
                            <div class="card h-100 shadow-sm border-0">
                                <img src="{{ $image->image_url }}" alt="Gallery Image" class="card-img-top rounded-top" style="height: 200px; object-fit: cover;">
                                <div class="card-body text-center">
                                    <h5 class="fw-bold">{{ $image->header }}</h5>
                                    <div class="d-flex justify-content-center gap-2 mt-2">
                                        
                                        <form action="{{ route('event.delete', $image->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this image?');">
                                            @csrf
                                            @method('DELETE')
